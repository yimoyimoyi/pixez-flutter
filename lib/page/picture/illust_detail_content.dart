...
class _IllustDetailContentState extends State<IllustDetailContent> {
  ...
  /// 标题/标签"显示译文"开关（手动触发语义：点击翻译后显示，再点切回原文）
  final Map<TranslateContentType, bool> _showTranslated = {};
  ...
  Future<void> _toggleShowTranslated(TranslateContentType type) async {
    ...
    if (_showTranslated[type] == true) {
      bool ok = true;
      switch (type) {
        case TranslateContentType.tag:
          ok = await _translationService.translateTags(_illusts);
          break;
        case TranslateContentType.title:
          ok = await _translationService.translateTitle(_illusts.title);
          break;
        case TranslateContentType.caption:
          break; // caption 由 SelectableHtml 内部自行触发
        case TranslateContentType.comment:
        case TranslateContentType.novelBody:
        case TranslateContentType.generic:
  ...
  bool get _titleTranslatedShown =>
  ...
  bool get _tagsPending {
    ...
    for (final tag in _illusts.tags) {
  ...
  /// 标签区批量翻译按钮（未开启标签翻译时不显示）
  Widget _tagTranslateButton() {
    final service = _translationService;
    if (!service.isTypeEnabled(TranslateContentType.tag)) {
    ...
    return TextButton.icon(
      ...
      onPressed: () => _toggleShowTranslated(TranslateContentType.tag),
    );
  }

  /// 通用"翻译/原文"切换按钮（标题/标签共用），未开启该类型时不显示
  Widget _translateButton(TranslateContentType type, String rawText) {
    final service = _translationService;
    if (!service.isTypeEnabled(type)) return const SizedBox.shrink();
    final shown = _showTranslated[type] ?? false;
    final pending = service.isPendingOf(rawText, type);
    return SizedBox(
      height: 32,
      child: TextButton.icon(
        ...
        onPressed: () => _toggleShowTranslated(type),
      ),
    );
  }
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                _translateButton(TranslateContentType.title, data.title),
              ],
            ),
          ),
          if (data.series != null)
            ...
            child: Container(
              ...
              child: SelectionArea(
                ...
                child: SelectableHtml(
                  ...
                  translateId: 'illust:${_illusts.id}',
                ),
              ),
            ),
          ),
